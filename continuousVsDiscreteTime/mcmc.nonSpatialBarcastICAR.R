...
mcmc <- function(WI, WP, HI, HP, params, method = 'continuous'){
  ...
  if(method == 'continuous'){
    ...
    mcmc.cont(WI = WI, WP = WP, HI = HI, HP = HP, params = params)
  } else if(method == 'discrete'){
    source('~/barcast/continuousVsDiscreteTime/mcmc.discrete.one.beta.R')
    mcmc.disc(WI = WI, WP = WP, HI = HI, HP = HP, params = params)
  } else{
    stop("not a valid method, use 'continuous' or 'discrete'")
  }
}
...
#     }
# 		A.tmp.chol <- chol(diag(A.tmp.vec + sigma.squared))
#     if(method == 'continuous'){
