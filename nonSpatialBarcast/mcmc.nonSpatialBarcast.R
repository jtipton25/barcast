...
mcmc <- function(WI, WP, n.mcmc, mu.0.tilde, sigma.squared.0.tilde, alpha.alpha, beta.alpha, mu.0, sigma.squared.0, alpha.sigma.squared, beta.sigma.squared, alpha.phi, beta.phi, alpha.I, beta.I, alpha.P, beta.P, mu.beta.0, sigma.squared.beta.0, mu.beta.1, sigma.squared.beta.1){
  ...
  for(l in 1:n.mcmc){
    ...
    #     b <- alpha * Sigma.epsilon.inv %*% (X[0 + 1, ] - (1 - alpha) * mu) + mu.0.tilde / sigma.squared.0.tilde
    b <- alpha / sigma.squared * (X[0 + 2, ] - (1 - alpha) * mu) + mu.0.tilde / sigma.squared.0.tilde
    X[0 + 1, ] <- rMVN(A.chol, b)
    ...
    tmp <- vector(length = t)
    for(i in 1:t){
      tmp[i] <- (1 - alpha)^2 / sigma.squared * J %*% (X[i + 1, ] - alpha * X[i, ])
    }
    ...
    for(i in 1:t){
      if(NIt[i] != 0){
        ...
        tmp[i] <- (WIt[[i]] - X[i + 1, 1]) %*% (WIt[[i]] - X[i + 1, 1])
      } else {
        tmp[i] <- 0
      }
    }
